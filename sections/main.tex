...
The two ambits of technology that we deem most worthy of future effort are (1) ways to extend current orchestrator capabilities beyond Cloud boundaries and (2) bringing the WebAssembly virtualisation to a mature level for usage in both high-powered Cloud nodes and resource-constrained Edge devices. 
Our empirical results show that present WebAssembly is nearly exclusively suited for pure compute functions, which is too little for the Continuum.
...
On the bright side, however, we trust these problems should be overcome given sufficient time and effort by the developers' community. On this account, we anticipate IoT/service orchestration and WebAssembly to receive significant attention in the coming years.
